...
projTime <- !is.null(values$projTimeMsk)
...
```{r, echo = projAny, eval = projAny, include = projAny}
projCoords <- data.frame(x = {{projAreaX}}, y = {{projAreaY}})
projPoly <- SpatialPolygons(list(Polygons(list(Polygon(projCoords)), ID=1)))
...
predsProj <- crop(preds, projPoly)
predsMsk <- mask(predsProj, projPoly)

newExtProj <- predict(modSel, predsMsk)
plot(newExtProj)
```
...
```{r, echo = projTime, eval = projTime, include = projTime}
predsFuture <- getData("CMIP5", var = "bio", res = {{predsRes}}, rcp = {{selRCP}}, model = "{{selGCM}}", year = {{selTime}})

predsProj <- crop(predsFuture, projPoly)
predsMsk <- mask(predsProj, projPoly)

# rename future climate variable names
names(predsMsk) <- names(preds)
futureProj <- predict(modSel, predsMsk)
plot(futureProj)
```
