...
# comp7
viz.pred.mtp <- rvs$comp7.thr == 'mtp'
viz.pred.p10 <- rvs$comp7.thr == 'p10'
viz.pred.raw <- rvs$comp7.type == 'raw'
viz.pred.log <- rvs$comp7.type == 'log'
viz.pred.cll <- rvs$comp7.type == 'cll'
viz.mapPred <- rvs$comp7.mapPred == TRUE
viz.mxEval <- !is.null(rvs$mxEvalSel)
viz.bcPlot <- !is.null(rvs$bcPlotsPar)
viz.resp <- rvs$comp7.resp == TRUE
# comp8
...
# read csv with coordinates for polygon
bgExt <- rgdal::readOGR("{{bgUserShpPath}}", "{{bgUserShpName}}")
```
...
occs.xy <- occs[,2:3]
bg.xy <- dismo::randomPoints(envsBgMsk, {{bgPtsNum}})  # generate 10,000 background points
...
e <- ENMeval::ENMevaluate(occs.xy, predsBgMsk, bg.coords=bg.xy, RMvalues=rms, fc={{fcs}}, method='user', occs.grp, bg.grp)
...
```{r mapPred, echo = viz.mapPred, include = viz.mapPred}
...
p <- predict(m, predsBgMsk, args=c("outputformat=logistic"))
```

```{r, echo = viz.pred.cll, include = viz.pred.cll}
# generate cloglog prediction
p <- predict(m, predsBgMsk, args=c("outputformat=cloglog"))
```

```{r, echo = viz.pred.mtp, include = viz.pred.mtp}
# get predicted values for occurrence grid cells
occVals <- raster::extract(p, occs.xy)
# define minimum training presence threshold
x <- thresh(occVals, 'mtp')
# threshold model prediction
p <- p > x
```

```{r, echo = viz.pred.p10, include = viz.pred.p10}
# get predicted values for occurrence grid cells
occVals <- raster::extract(p, occs.xy)
# define 10% training presence threshold
x <- thresh(occVals, 'p10')
# threshold model prediction
p <- p > x
```

```{r, echo = viz.mapPred, include = viz.mapPred}
