...
# find row numbers that match IDs to be removed
occs.rem <- which(occs$occID %in% {{occsRemoved}})
...
p <- dismo::predict(m, predsBgMsk, args=c("outputformat=logistic"))
```

```{r, echo = viz.pred.cll, include = viz.pred.cll}
# generate cloglog prediction
p <- dismo::predict(m, predsBgMsk, args=c("outputformat=cloglog"))
```
