...
# comp6
mod.bioclim <- rvs$enmSel == 'bc'
mod.maxent <- rvs$enmSel == 'maxent'
mod.any <- mod.bioclim | mod.maxent
# comp7
viz.pred.mtp <- rvs$predThresh == 'mtp'
viz.pred.p10 <- rvs$predThresh == 'p10'
viz.pred.raw <- rvs$predType == 'raw'
viz.pred.log <- rvs$predType == 'log'
viz.pred.cll <- rvs$predType == 'cll'
# comp8
pj.area <- rvs$pjType == 'area'
pj.time <- rvs$pjType == 'time'
pj.mess <- !is.null(rvs$mess)
```
...
```{r partSwitch, include = FALSE, eval = part}
