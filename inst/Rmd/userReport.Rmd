...
```{asis, echo = occ.csv, eval = occ.csv, include = occ.csv}
### Obtain Occurrence Data
```

```{asis, echo = occ, eval = occ, include = occ}
...
```{r occSearch, echo = occ, include = occ}
...
```{asis, echo = csv, eval = csv, include = csv}
...
```{asis, echo = thin | remLoc | drawPoly, eval = drawPoly, include = drawPoly}
...
```{r remLocs, echo = remLoc, include = remLoc}
...
```{r doThin2, echo = thin, include = thin}
...
```{r getPreds, echo = yesPreds, include = yesPreds}
...
```{r backgBuf, echo = isBB, include = isBB}
xmin <- min(occs$longitude) - ({{backgBuf}} + raster::(preds)[1])
xmax <- max(occs$longitude) + ({{backgBuf}} + raster::(preds)[1])
ymin <- min(occs$latitude) - ({{backgBuf}} + raster::(preds)[1])
ymax <- max(occs$latitude) + ({{backgBuf}} + raster::(preds)[1])
bb <- matrix(c(xmin, xmin, xmax, xmax, xmin, ymin, ymax, ymax, ymin, ymin), ncol=2)
backgExt <- sp::SpatialPolygons(list(sp::Polygons(list(sp::Polygon(bb)), 1)))
```

```{r backgBuf2, echo = isMCP, include = isMCP}
xy_mcp <- mcp(occs[,2:3])
bb <- xy_mcp@polygons[[1]]@Polygons[[1]]@coords
backgExt <- rgeos::gBuffer(xy_mcp, width = {{backgBuf}} + raster::(preds)[1])
```
...
```{r userBackgPoly, echo = isUserBG, include = isUserBG}
...
```{r userBackgPoly2, echo = isUserBG, include = isUserBG}
shp <- sp::SpatialPolygons(list(sp::Polygons(list(sp::Polygon(shp)), 1)))
shp <- rgeos::gBuffer(shp, width = {{backgBuf}} + raster::(preds)[1])
backgExt <- shp
...
```{r predsMask, echo = backg, include = backg}
