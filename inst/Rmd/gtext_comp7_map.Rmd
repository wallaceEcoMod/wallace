...
Users must first choose a model. Depending on the `ENMeval` settings selected in Component **Model**, there may be multiple choices for Maxent. *Wallace* calculates model output based on the predictor variable values for each cell and plots the prediction on the map. Two Maxent model output scales are available for viewing in *Wallace*: "raw", "logistic", and "cloglog". The "raw" values sum to 1 across the entire study extent and represent the relative occurrence rate (*not* probability of presence; see Merow et al. 2013). The "logistic" transformation of the "raw" values range from 0 to 1 and approximate probability of presence (with strong caveats that include assumptions about prevalence; see Yackulic et al. 2012 and Merow et al. 2013). The "cloglog" transformation of the "raw" values also ranges from 0 to 1, and approximates probability of presence as well (relying on different assumptions than the "logistic" transform).
