...
*Wallace* provides two simple ways to delimit a study region, or accepts a user-specified polygon in .csv format (with latitude/longitude coordinates of polygon vertices). For the former, users can choose either a: 1) bounding box (rectangle with most the extreme coordinates in the four cardinal directions as vertices); or a 2) minimum convex polygon (a convex shape drawn around localities with minimized area), as well as a buffer distance (in degrees; i.e. not meters). *Wallace* then masks the environmental grids by the chosen polygon. Users can download the masked grids as two raster grid formats (.grd and .asc), or as an image file (.png).
...
Peterson A. T., Soberón J., Pearson R. G., Anderson R. P., Martinez-Meyer E., Nakamura M., Araújo M. B. (2011). Niches and Geographic Distributions. In: *Ecological Niches and Geographic Distributions*. Princeton, New Jersey: Monographs in Population Biology, 49. Princeton University Press.
