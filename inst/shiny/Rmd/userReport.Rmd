...
occs.{{sp}}.z <- as.data.frame(raster::extract(envs.{{sp}}, occs.{{sp}}.xy))
# remove occurrence records with NA environmental values
occs.{{sp}} <- remEnvsValsNA(occs.{{sp}}, occs.{{sp}}.z)
# also remove variable value rows with NA environmental values
occs.{{sp}}.z <- na.omit(occs.{{sp}}.z)
# add columns for env variable values for each occurrence record
occs.{{sp}} <- cbind(occs.{{sp}}, occs.{{sp}}.z)
```
...
bg.{{sp}}.z <- as.data.frame(raster::extract(bgMask.{{sp}}, bg.{{sp}}))
```
...
pca.{{sp}} <- cESpace_pca("{{espace.sp1}}", "{{espace.sp2}}", 
                          occs.{{espace.sp1}}.z[pca.sel], 
                          occs.{{espace.sp2}}.z[pca.sel],
                          bg.{{espace.sp1}}.z[pca.sel], 
                          bg.{{espace.sp2}}.z[pca.sel])
```
