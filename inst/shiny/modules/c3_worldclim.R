c3_worldclim_module_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
c3_worldclim_module_server <- function(input, output, session, common) {
  ...
  observeEvent(input$goEnvData, {
    ...
    # FUNCTION CALL ####
    wcbc <- c3_worldclim(input$wcRes, input$bcSel, mapCntr(), input$doBrick, logger)
    req(wcbc)
...
c3_worldclim_module_map <- function(map, common) {
  # Map logic
  occs <- common$occs
  map %>% clearAll() %>%
...
c3_worldclim_module_rmd <- function(species) {
