...
profileOccs_UI <- function(id) {
  ...
  tagList(
    ...
    tags$div(
      ...
      checkboxGroupInput(ns("grades"), label = strong("Choose grades to keep"), choices = list("A: xxx" = "A",
                                                                                               "B: xxx" = "B",
                                                                                               "C: xxx" = "C",
                                                                                               "D: xxx" = "D",
                                                                                               "E: xxx" = "E",
    ...
    for(sp in spLoop) {
      ...
      output$profileOccsResults <- renderUI({
        ...
        output$profOccsBarplot <- renderPlot({
          ...
          ggplot2::ggplot(ggplot2::aes(x=quality.grade,fill=quality.grade), data = full.qaqc) + ggplot2::geom_bar() 
        })
