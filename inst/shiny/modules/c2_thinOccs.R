c2_thinOccs_module_ui <- function(id) {
  ns <- shiny::NS(id)
  tagList(
    tags$p(
...
c2_thinOccs_module_server <- function(input, output, session, common) {
...
c2_thinOccs_module_map <- function(map, common) {
  spp <- common$spp
  curSp <- common$curSp
  occs <- common$occs
  # Map logic
...
# c2_thinOccs_module_rmd <- function(species) {
