c1_userOccs_module_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
c1_userOccs_module_server <- function(input, output, session, common) {
  logger <- common$logger
  spp <- common$spp

  observeEvent(input$goUserOccs, {
    # FUNCTION CALL ####
    occsList <- c1_userOccs(input$userCSV$datapath, input$userCSV$name, logger)
...
c1_userOccs_module_map <- function(map, common) {
  spp <- common$spp
  curSp <- common$curSp
...
c1_userOccs_module_rmd <- function(species) {
  list(
