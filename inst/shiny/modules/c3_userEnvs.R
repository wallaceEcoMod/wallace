c3_userEnvs_module_ui <- function(id) {
  ns <- NS(id)
  tagList(
...
c3_userEnvs_module_server <- function(input, output, session, common) {
...
c3_userEnvs_module_result <- function(id) {
  ns <- NS(id)
  # Result UI
  verbatimTextOutput(ns("envsPrint"))
}

c3_userEnvs_module_map <- function(map, common) {
  occs <- common$occs
  map %>% clearAll() %>%
