...
mapProjVals_{{spAbr}} <- getRasterVals(proj_area_{{spAbr}}$projArea,"{{outputType_rmd}}")
...
# map model prediction raster and projection polygon
  ...
  addRasterImage(proj_area_{{spAbr}}$projAime, colors = rasPal_{{spAbr}}, opacity = 0.7,
...
proj_area_{{spAbr}} <- proj_area_{{spAbr}}$projArea > {{threshold_rmd}}
...
bbZoom <- polyZoom(bb_{{spAbr}}[1, 1], bb_{{spAbr}}[2, 1], bb_{{spAbr}}[1, 2], 
                   bb_{{spAbr}}[2, 2], fraction = 0.05)
mapProjVals_{{spAbr}} <- getRasterVals(proj_area_{{spAbr}},"{{outputType_rmd}}")
