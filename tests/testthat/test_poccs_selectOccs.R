...
### run function
out.map <- poccs_selectOccs(occs, polySelXY = expertAddedPoly)


### test if the error messages appear when they are supposed to
test_that("error checks", {
  # the user has not obtained or loaded the occurrence data
  expect_error(poccs_selectOccs(occs = NULL, polySelXY = expertAddedPoly),
               'Before processing occurrences, obtain the data in component 1.')
  # the user has not finished the polygon
  expect_error(poccs_selectOccs(occs, polySelXY = NULL),
