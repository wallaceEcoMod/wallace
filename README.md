...
remotes::install_github("wallaceEcoMod/wallace@biomodelos")
library(wallace)
run_wallace()
```
...
*Wallace* v.2026.05.05 includes two options to run Maxent models: maxnet and maxent.jar. The former, which is an R implementation and fits the model with the package `glmnet`, is now the default and does not require the package `rJava` (see Phillips et al. 2017).
