...
correlations_module_server <- function(input, output, session, common) {
  ...
  observeEvent(input$runCorrs, {
    ...
    logger %>% writeLog(alfred.hlSpp(sp), "Finished calculating correlations")
  ...
  observeEvent(input$selectConfirm, {
    ...
    logger %>% writeLog(alfred.hlSpp(curSp()), "Selected: ", paste0(names(spp[[curSp()]]$procEnvs$bgMask), collapse = ", "))
  })
}
```
