...
## *Wallace* modules (v1.9.1)

```{r wallace logo, echo = FALSE, fig.width=4, fig.height=3, fig=TRUE, eval = TRUE}
...
alfred.smartProgress(shinyLogs, message = "calculating pairwise Pearson's correlations",{
corrs <- layerStats(x, stat = "pearson", na.rm = TRUE)
})
shinyLogs %>% writeLog("Pearson complete")
return(corrs)
}
```
<!-- CBM: why is na.rm an argument of the function? I added na.rm = TRUE in layerStats() so the correlations could be calculated. -->
...
runCorr_UI <- function(id) {
  ns <- NS(id)
  tagList(tags$div(title="Calculate pairwise Pearson's Correlation matrix for 
...
runCorr_MOD <- function(input, output, session) {
  reactive({
    ...
    if(is.null(spp[[curSp()]]$procEnvs$bgMask)) {
      ...
      "Before calculating correlations, please mask environmental data for", 
      curSp(), ".")
      return()
    }
    # FUNCTION CALL ####
    # run the module function from /wallace/R
    mod.Corr <- wallacelayerStats(x = spp[[curSp()]]$procEnvs$bgMask, asSample = TRUE,
                                  na.rm = FALSE, shinyLogs = shinyLogs)
