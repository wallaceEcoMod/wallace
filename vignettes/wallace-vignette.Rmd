...
# Preface
This vignette and any others in the `wallace` package will be updated regularly in accordance with ongoing development. 

# Introduction
*Wallace* v1.0.4 is an `R`-based GUI application that currently offers functionality for modeling species niches and distributions. We will refer to these models as species distribution models (SDMs), and we will not explain them at length here: the purpose of this vignette is to show users how to use *Wallace*. As you read through, you will be pointed to some sources of detailed info within the application for reference. In the future, *Wallace* will likely include other ecological modeling techniques.

*Wallace* has many qualities which we think make it a good example of next-generation scientific software: it is 1) open, 2) expandible, 3) flexible, 4) interactive, 5) instructive, and 6) reproducible. The application features a pannable/zoomable map and dynamic plots and tables. Occurrence and environmental data for modeling can be downloaded from online databases or uploaded by the user. Most analysis results can be downloaded, and there is an option to download R code that can reproduce your analysis. For more details, including on SDMs, please see our [publication](http://onlinelibrary.wiley.com/doi/10.1111/2041-210X.12945/full) in Methods in Ecology and Evolution. The citation is below:
...
The *Wallace* project's [main page](https://wallaceecomod.github.io/) has some basic information and links, which are listed below: 

- [Google Group](https://groups.google.com/forum/#!forum/wallaceecomod)  
- official [email](mailto:wallaceecomod@gmail.com)  
- stable version on [CRAN](https://CRAN.R-project.org/package=wallace)  
- development versions on [Github](https://github.com/wallaceEcoMod/wallace)  
