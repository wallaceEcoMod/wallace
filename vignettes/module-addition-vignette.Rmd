...
author: "Jamie Kass, Gonzalo Pinilla-Buitrago, Siew Fong Chen, Peter Galante, Bethany Johnson, Dean Attali"
date: "April 19, 2022"
...
7. Tell the Wallace app to use your new module.  
...
* rmd (boolean): FALSE for this example. TRUE if the module should add Rmd code to the Session Code. See more in Step 5, option *v*.
* save (boolean): FALSE for this example.  TRUE if the module saves custom data when the user saves the current session. See more in Step 5, option *vi*.
...
v) *OPTIONAL:* A function named `penvs_correlations_module_rmd` that adds code to the “Session Code” Rmd output to enable reproducibility of your module analysis. This function returns a list with variables that are used in the module's RMD code, and takes an argument `species` that contains all the information for the current species. 
...
### Step 7: Tell the Wallace app to use your module
