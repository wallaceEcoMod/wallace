...
Let's first write a function that carries out the module's analysis. This function takes the following inputs: environmental predictor values at 1) occurrence points and 2) background points, 3) degrees of freedom for the spline smoothing terms in the GAM, 4) the species name, and 5) shinyLogs. The last one, shinyLogs, is a reactive value (`shiny::reactiveVal()`) that holds all the text and HTML formatting for the log window. If you call the function with shinyLogs as a parameter input, certain functionality inside will be `shiny`-functional. If shinyLogs is not NULL, `writeLog()` will and `smartProgress()` functions. If 
