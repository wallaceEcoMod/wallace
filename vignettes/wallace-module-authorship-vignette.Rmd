...
*Wallace* is an R-based, interactive application for reproducible modeling of species niches
and distributions. The application is also expandible via the addition of new modules, which in *Wallace* are discrete parts of the modeling analysis. This vignette steps through the module authorship process, focusing on the steps involved in adding a module, and also on best practices and some troubleshooting.

## Module structure

This section was written for *Wallace* v1.0.5. There are xx necessary steps to adding a functional module. This example will be for a module that implements a binomial generalized additive model (GAM) for presence-only occurrence data.

1. Write a function that carries out the module's analysis.

```{r}
model_gam <- function(occs.xy, bg.xy, envs, df) {
