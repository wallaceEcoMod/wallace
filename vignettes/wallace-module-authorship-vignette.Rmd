...
Let's first write a function that carries out the module's analysis. This function takes the following inputs: environmental predictor values at 1) occurrence points, 2) background points, 3) degrees of freedom for the spline smoothing terms in the GAM, 4) the species name, and 5) shinyLogs. The last one, shinyLogs, is a reactive value (`shiny::reactiveVal()`) that holds all the text and HTML formatting for the log window. 
<!-- cm:define reactive value -->
If you call the function with shinyLogs as a parameter input, certain functionality inside will be `shiny`-functional. If shinyLogs is not NULL, `writeLog()` will 
<!-- cm: will what? -->
and `smartProgress()` functions. If 
<!-- cm: ummm -->
...
Before moving forward, we should go over some important points about the structure of the data that *Wallace* stores on species. All data specific to a species is recorded in a reactiveValues list, the reactive version of a regular list, called `spp`. To access the data on the species currently selected, we index `spp` with a reactive function that returns the selected species' name, called `curSp()` in this way: `spp[[curSp()]]`. This gives us a list of data objects for the selected species.
<!-- CM: show an example of this spp -->
<!-- CM: not clear here why you need functions to access list elements -->
A complete list of all reactive functions that return character outputs is below.
...
In the module script, we define two main functions, `runGAM_UI()` and `runGAM_MOD()`, which specify the logic for `ui.R` and `server.R`, respectively. The function `runGAM_UI()` details the UI controls, including hover text if needed, etc. One thing that is perhaps unexpected is the use of the `NS()` function, which ensures these UI controls can talk to `ui.R` even though they are in their own namespace. It turns out `shiny` modules exist in unique namespaces, and this means that if we want to use any of the results inside here in `server.R`, we need to funnel them into our `spp` list. We can see this below, in the `runGAM_MOD()` function. 

This regular function is a wrapper for a `shiny` reactive expression. Inside, there is a template for the essential parts needed in a *Wallace* module. The first, "ERRORS", catches any possible errors before the module's core functionality is run, writes a log message, then returns to `server.R`. The user will need to define these. Next, we call the function from `/wallace/R` and make sure it returned something. Next, we load the model object into `spp`. Finally, we fill in the right metadata fields for the rangeModelMetadata object in `spp`.

There are two other functions below this. The first is `modGAM_INFO()`, which includes information on the packages used in the module for the UI around the controls. It is called in `ui.R`. The second is `modGAM_TBL()`, which defines a custom UI with `shiny::renderUI()` that shows the model `summary()` output from the console, and assigns it to `output$evalTbls`, which is a built-in UI for model result displays. It is called in `server.R`.

Save this as a script here `wallace/inst/shiny/modules/mod_runGAM.R`.

<!-- cm: what is the id arg? -->
```{r}
...
Next, the function for our new module's ui must be referenced into the file `ui.R` in order for *Wallace* to display the controls and package information. Open `ui.R` and navigate to the MODEL component. We will be adding some code to the existing code block within `shiny::conditionalPanel()`. This function displays UI only if the logical statement in the first parameter is met, which in this case is the user selection of the "Model" component tab. We'll first be adding a new radio button for selection of the GAM module, then creating a new `conditionalPanel()` block below BIOCLIM that specifies all the UI necessary for our module. Most of this is handled by `runGAM_UI()`, which we defined in `mod_runGAM.R` above. We also use the internal functions `uiTop()` (in this block) and `uiBottom()` (below the download button) to display the package information we defined in `runGAM_INFO()`. Lastly, we add the `shiny::actionButton()` for our module to enable the user to run it. We'll see how we can observe this button for a press in order to activate our module at the right time in the next step. In the code below, please see the comments for where we made changes to the original `ui.R`.
...
Finally, we need to call the module and the other functions from `mod_runGAM.R` in `server.R`. Open `server.R` and navigate to the MODEL component. You should see "module Maxent" and "module BIOCLIM" code blocks, wrapped in `observeEvent()`. This `shiny` function initiates what's inside the block when an action is taken by the user, which in this case is a button press. The button we are observing is `input$goGAM`, which we just specified in `ui.R`. In `server.R`, entities from `ui.R` have the notation `input$...`. Within this block, we will call our module with `shiny::callModule()`, which assigns the reactive expression to a variable, and then initialize it by running the module function on the next line. We do not assign it to a variable because our modules have no output---all the assignments are internal and to the list `spp`. We then check to make sure the results were saved, then run our specialized table function, which displays the model results. Finally, we make some UI updates (which can only be made in `server.R`) to switch to the Results tab automatically, and to update the module selection in the next component to display only those relevant to GAMs (which for now is just "Map Prediction").
...
The last step, and one which is vital, is writing guidance text for your module for the benefit of other users. There are three categories for guidance text. BACKGROUND provides general information about the framing of the module's technique in the grand scheme of things. IMPLEMENTATION talks about how the technique is used in this module. Finally, REFERENCES is a bibliography of academic sources that are referenced in the text.
