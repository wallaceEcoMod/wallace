...
```{asis, echo = thin | remLoc | drawPoly}
...
```{asis, echo = drawPoly, eval = drawPoly}
You drew a polygon. It was stunning, a real work of modern art. Have you thought of a career change? These occurrence localities were selected: `r values$ptSeln`.
```

```{r, echo = drawPoly, eval = drawPoly}
occs <- occs[values$ptSeln, ]  # subset occs by selected rows
```
