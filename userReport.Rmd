...
Please find below the R code history from your *Wallace* session. You can reproduce your session results by running the .Rmd file in RStudio (for more information see <http://rmarkdown.rstudio.com>).
...
Wallace also includes several functions developed to help integrate different packages and some additional functionality. For this reason, it is necessary to load the file 'functions.R', which can be found on Wallace's GitHub page (https://github.com/wallaceEcoMod/wallace). Download the file, place it in your working directory (find this with `getwd()`), and then load it:
```{r loadFunctions}
...
respCurv <- !is.null(values$respCurvParams)
bcEnvelPlot <- !is.null(values$bcEnvelPlot)
mxEvalPlot <- !is.null(values$mxEvalPlot)
...
output <- thin(occs, 'latitude', 'longitude', 'name', thin.par = {{thinDist}}, reps = 100, locs.thinned.list.return = TRUE, write.files = FALSE, verbose = FALSE)
```
...
predsBackgCrop <- crop(preds, backgExt)
predsBackgMsk <- mask(predsBackgCrop, backgExt)
```
...
occs.locs <- occs[,2:3]
bg.coords <- randomPoints(predsBackgMsk, 10000)  # generate 10,000 background points
...
group.data <- get.checkerboard1(occ=occs.locs, env=predsBackgMsk, bg.coords=bg.coords, aggregation.factor={{aggFact}})
```

```{r, echo = isCB2, eval = isCB2, include = isCB2}
group.data <- get.checkerboard2(occ=occs.locs, env=predsBackgMsk, bg.coords=bg.coords, aggregation.factor={{aggFact}})
```
...
e <- ENMevaluate(modParams$occ.pts, predsBackgMsk, bg.coords=modParams$bg.pts, RMvalues=rms, fc={{fcsSel}}, method='user', occ.grp=modParams$occ.grp, bg.grp=modParams$bg.grp)
evalTbl <- e@results
evalMods <- e@models
...
```{r, echo = respCurv, eval = respCurv, include = respCurv}
dismo::response(evalMods[[{{respCurvParamsMod}}]], var = "{{respCurvParamsVar}}")
```

```{r, echo = bcEnvelPlot, eval = bcEnvelPlot, include = bcEnvelPlot}
plot(evalMods[[1]], a = {{bcPlot1}}, b = {{bcPlot2}}, p = {{bcPlotP}})
```

```{r, echo = mxEvalPlot, eval = mxEvalPlot, include = mxEvalPlot}
eval.plot(evalTbl, value = "{{mxEvalPlotSel}}")
```

```{asis, echo = projArea}
### Project Niche Model
You selected to project your model to a new geographic area. First define a polygon with the coordinates you chose, then crop and mask your predictor rasters. Finally, predict suitability values for these new raster cells based on the model you selected.
```

```{r, echo = projArea, eval = projArea, include = projArea}
projAreaDf <- data.frame(x = {{projAreaX}}, y = {{projAreaY}})
projAreaPoly <- SpatialPolygons(list(Polygons(list(Polygon(projAreaDf)), ID=1)))

predsPjAreaCrop <- crop(preds, projAreaPoly)
predsPjAreaMsk <- mask(predsPjAreaCrop, projAreaPoly)

modSel <- evalMods[[{{modSel}}]]
pjArea <- predict(modSel, predsPjAreaMsk)
```
