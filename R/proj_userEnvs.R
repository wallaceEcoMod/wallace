...
#'                                 aggFact = NULL)
#' userExt <- rgdal::readOGR(system.file("extdata/shp", package = "wallace"))
...
proj_userEnvs <- function(evalOut, curModel, envs, pjExt, alg, outputType = NULL,
  ...
  if (alg == 'BIOCLIM') {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'User specified projection for BIOCLIM model.')
  } else if (alg == 'maxent.jar' | clamp == TRUE) {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'User specified projection for clamped model ', curModel, '.')
  } else if (clamp == FALSE) {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'User specified projection for unclamped model', curModel, '.')
  }
