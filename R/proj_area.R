...
proj_area <- function(evalOut, curModel, envs, pjExt, alg, outputType = NULL,
  ...
  if (alg == 'BIOCLIM') {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'New area projection for BIOCLIM model.')
  } else if (alg == 'maxent.jar'| clamp == TRUE) {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'New area projection for clamped model ', curModel, '.')
  } else if (clamp == FALSE) {
    logger %>% alfred.writeLog(alfred.hlSpp(spN), 'New area projection for unclamped model ', curModel, '.')
  }
